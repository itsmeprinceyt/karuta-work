...
import { InvalidInput, NoBitsFound, NotTriggeredByYouBits } from "../utils/karuta-bits";
import { UserBitsSession } from "../types/BitsData.type";

const userBitsSessions = new Map<string, UserBitsSession>();
...
export default {
    ...
    async execute(message: Message) {
        ...
        if (!triggeredByUser) {
            await message.reply({
                embeds: [NotTriggeredByYouBits(), ytEmbed],
                components: [ytButton],
            });
            return;
        }
...
export async function handleMessageUpdate(oldMessage: Message, newMessage: Message) {
    if (!newMessage.author.bot || newMessage.author.id !== Karuta) return;
    if (!newMessage.embeds[0]?.description?.includes('Bits carried by')) return;
    ...
    if (!userSession || userSession.isComplete) return;
        ...
        if (isLastPage) {
            ...
            if (userSession.lastMessageId) {
                try {
                    ...
                    await (newMessage.channel as TextChannel).send(summaryText);
                }
            } else {
                await (newMessage.channel as TextChannel).send(summaryText);
            }
            ...
            if (userSession.lastMessageId) {
                ...
                const sent = await (newMessage.channel as TextChannel).send(summaryText);
                userSession.lastMessageId = sent.id;
            }
        }
    }
...
function parsePaginationInfo(footerText: string): { currentPage: number; totalPages: number } {
...
function parseBitsFromEmbed(embed: any): { bits: Map<string, number>, username: string } {
...
function createFinalBitsSummaryText(session: UserBitsSession, isLastPage = false): string {
    const bitsArray = Array.from(session.bits.entries())
        .sort(([, quantityA], [, quantityB]) => quantityB - quantityA);

    const bitsString = bitsArray.map(([bitName, quantity]) =>
        `${quantity.toLocaleString().replace(/,/g, '')} ${bitName}`
    ).join(', ');
...
function createRatioSummaryText(session: UserBitsSession, ratio: number, isLastPage = false): string {
    const bitsArray = Array.from(session.bits.entries())
        .sort(([, quantityA], [, quantityB]) => quantityB - quantityA);
    ...
    const bitsString = adjustedBitsArray.map(([bitName, quantity]) =>
        `${quantity.toLocaleString().replace(/,/g, '')} ${bitName}`
    ).join(', ');
